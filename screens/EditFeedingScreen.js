...
const EditFeedingScreen = () => {
  ...
  const fetchActivity = async () => {
    try {
      ...
      if (error) {
        ...
        Alert.alert('Error', 'Failed to load feeding details');
        return;
      }
      
      if (!data) {
        Alert.alert('Error', 'Feeding record not found');
        navigation.goBack();
        return;
      }
      
      if (data.activity_type !== 'feeding') {
        Alert.alert('Error', 'This is not a feeding activity');
        navigation.goBack();
        return;
      }
  ...
  const handleSaveActivity = async () => {
    try {
      ...
      // Prepare feeding data for update
      const feedingData = {
      ...
      if (error) {
        ...
        Alert.alert('Error', 'Failed to update feeding');
        return;
      }
      ...
      Alert.alert(
        ...
        [
          { 
            text: "OK", 
            onPress: () => navigation.navigate('FeedingDetail', { activityId }) 
          }
        ]
      );
    } catch (error) {
