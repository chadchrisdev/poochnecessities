...
DECLARE
    ...
    has_data boolean;
    r record; -- Define r as a record variable for the loops
BEGIN
    ...
    IF table_exists THEN
        ...
        DROP TABLE public.weights CASCADE; -- Add CASCADE to drop dependencies too
    END IF;
    ...
    IF table_exists AND has_data THEN
        ...
        DECLARE
            insert_columns text := '';
            select_columns text := '';
            col_r record; -- New record variable for this loop
        BEGIN
            FOR col_r IN 
            ...
            LOOP
                ...
                END IF;
                insert_columns := insert_columns || col_r.column_name;
                select_columns := select_columns || col_r.column_name;
            END LOOP;
